...
public class AccessibleActionExample {
	...
	public static void main(final String[] args) {
		...
		final Canvas customButton = new Canvas(shell, SWT.NONE) {
			...
			public Point computeSize(final int wHint, final int hHint, final boolean changed) {
				...
				final Point point = gc.stringExtent(buttonText);
				gc.dispose();
				point.x += MARGIN;
				point.y += MARGIN;
				return point;
		...
		customButton.addPaintListener(e -> {
			final Rectangle clientArea = customButton.getClientArea();
			final Point stringExtent = e.gc.stringExtent(buttonText);
			final int x = clientArea.x + ((clientArea.width - stringExtent.x) / 2);
			final int y = clientArea.y + ((clientArea.height - stringExtent.y) / 2);
			e.gc.drawString(buttonText, x, y);
		});
		customButton.addMouseListener(MouseListener.mouseDownAdapter(e -> {
			final int actionIndex = (e.button == 1) ? 0 : 1;
			customButtonAction(actionIndex);
		}));
		customButton.addKeyListener(new KeyAdapter() {
			@Override
			public void keyPressed(final KeyEvent e) {
				final int modifierKeys = e.stateMask & SWT.MODIFIER_MASK;
		...
		final Accessible accessible = customButton.getAccessible();
		...
		accessible.addAccessibleActionListener(new AccessibleActionAdapter() {
			@Override
			public void getActionCount(final AccessibleActionEvent e) {
				e.count = 2;
			}
			@Override
			public void getName(final AccessibleActionEvent e) {
			...
			@Override
			public void getDescription(final AccessibleActionEvent e) {
				if ((0 <= e.index) && (e.index <= 1)) {
					e.result = AccessibleActionExample.getResourceString("action" + e.index + "description");
				}
			}
			@Override
			public void doAction(final AccessibleActionEvent e) {
				if ((0 <= e.index) && (e.index <= 1)) {
			...
			@Override
			public void getKeyBinding(final AccessibleActionEvent e) {
				switch (e.index) {
	 ...
	 */
	static void customButtonAction(final int action) {
