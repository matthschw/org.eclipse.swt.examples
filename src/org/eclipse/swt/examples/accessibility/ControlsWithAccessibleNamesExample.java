...
public class ControlsWithAccessibleNamesExample {
	...
	public static void main(final String[] args) {
		...
		final ImageData source = smallImage.getImageData();
		...
		final AccessibleAdapter overrideAccessibleAdapter = new AccessibleAdapter() {
			@Override
			public void getName(final AccessibleEvent e) {
				final Control control = ((Accessible) e.getSource()).getControl();
...
//		Table table, overrideTable;
		...
		for (int col = 0; col < 3; col++) {
			final TableColumn column = new TableColumn(overrideTable, SWT.NONE);
			column.setText("Col " + col);
			column.pack();
		}
		for (int row = 0; row < 3; row++) {
			final TableItem item = new TableItem(overrideTable, SWT.NONE);
			item.setText(new String [] {"C0R" + row, "C1R" + row, "C2R" + row});
		}
...
//		Tree tree, overrideTree;
		...
		for (int i = 0; i < 3; i++) {
			final TreeItem item = new TreeItem(overrideTree, SWT.NONE);
			item.setText("Item" + i);
			for (int j = 0; j < 4; j++) {
				new TreeItem(item, SWT.NONE).setText("Item" + i + j);
			}
...
//		Tree treeTable, overrideTreeTable;
		...
		for (int col = 0; col < 3; col++) {
			final TreeColumn column = new TreeColumn(treeTable, SWT.NONE);
			column.setText("Col " + col);
			column.pack();
		}
		for (int i = 0; i < 3; i++) {
			final TreeItem item = new TreeItem(treeTable, SWT.NONE);
		...
		for (int col = 0; col < 3; col++) {
			final TreeColumn column = new TreeColumn(overrideTreeTable, SWT.NONE);
			column.setText("Col " + col);
			column.pack();
		}
		for (int i = 0; i < 3; i++) {
			final TreeItem item = new TreeItem(overrideTreeTable, SWT.NONE);
			item.setText(new String [] {"I" + i + "C0", "I" + i + "C1", "I" + i + "C2"});
			for (int j = 0; j < 4; j++) {
				new TreeItem(item, SWT.NONE).setText(new String [] {"I" + i + j + "C0", "I" + i + j + "C1", "I" + i + j + "C2"});
			}
...
//		ToolBar toolBar, overrideToolBar;
		...
		for (int i = 0; i < 3; i++) {
			final ToolItem item = new ToolItem(overrideToolBar, SWT.PUSH);
			item.setText("Item" + i);
			item.setToolTipText("ToolItem ToolTip" + i);
		}
...
//		ToolBar imageToolBar, overrideImageToolBar;
		...
		for (int i = 0; i < 3; i++) {
			final ToolItem item = new ToolItem(imageToolBar, SWT.PUSH);
		...
		for (int i = 0; i < 3; i++) {
			final ToolItem item = new ToolItem(overrideImageToolBar, SWT.PUSH);
			item.setImage(transparentImage);
			item.setToolTipText("Image ToolItem ToolTip" + i);
		}
...
//		CoolBar coolBar, overrideCoolBar;
		...
		for (int i = 0; i < 2; i++) {
			...
			final ToolBar coolItemToolBar = new ToolBar(coolBar, SWT.FLAT);
			int toolItemWidth = 0;
			for (int j = 0; j < 2; j++) {
				final ToolItem item = new ToolItem(coolItemToolBar, SWT.PUSH);
				item.setText("I" + i + j);
          ...
          toolItemWidth = item.getWidth();
			...
			final Point size = coolItemToolBar.computeSize(SWT.DEFAULT, SWT.DEFAULT);
		...
		for (int i = 0; i < 2; i++) {
			final CoolItem coolItem = new CoolItem(overrideCoolBar, SWT.PUSH);
			final ToolBar coolItemToolBar = new ToolBar(overrideCoolBar, SWT.FLAT);
			int toolItemWidth = 0;
			for (int j = 0; j < 2; j++) {
				final ToolItem item = new ToolItem(coolItemToolBar, SWT.PUSH);
				item.setText("I" + i + j);
...
//		TabFolder tabFolder, overrideTabFolder;
		...
		for (int i = 0; i < 3; i++) {
			...
			final Text itemText = new Text(tabFolder, SWT.MULTI | SWT.BORDER);
			itemText.setText("\nText for TabItem " + i + "\n\n");
		...
		for (int i = 0; i < 3; i++) {
			final TabItem item = new TabItem(overrideTabFolder, SWT.NONE);
			item.setText("TabItem &" + i);
			item.setToolTipText("TabItem ToolTip" + i);
			final Text itemText = new Text(overrideTabFolder, SWT.MULTI | SWT.BORDER);
			itemText.setText("\nText for TabItem " + i + "\n\n");
			item.setControl(itemText);
		}
...
//		CTabFolder cTabFolder, overrideCTabFolder;
		...
		for (int i = 0; i < 3; i++) {
			...
			item.setToolTipText("TabItem ToolTip" + i);
			final Text itemText = new Text(cTabFolder, SWT.MULTI | SWT.BORDER);
			itemText.setText("\nText for CTabItem " + i + "\n\n");
			item.setControl(itemText);
		}
		...
		for (int i = 0; i < 3; i++) {
			final CTabItem item = new CTabItem(overrideCTabFolder, SWT.NONE);
			item.setText("CTabItem &" + i);
			item.setToolTipText("TabItem ToolTip" + i);
			final Text itemText = new Text(overrideCTabFolder, SWT.MULTI | SWT.BORDER);
			itemText.setText("\nText for CTabItem " + i + "\n\n");
			item.setControl(itemText);
		}
