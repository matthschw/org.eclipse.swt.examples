...
public class AccessibleTableExample {
	...
	public static void main(final String[] args) {
		...
		for (int col = 0; col < 3; col++) {
			final CTableColumn column = new CTableColumn(table1, SWT.NONE);
			column.setText("Col " + col);
			column.setWidth(50);
		}
		for (int row = 0; row < 4; row++) {
			final CTableItem item = new CTableItem(table1, SWT.NONE);
			item.setText(new String [] {"C0R" + row, "C1R" + row, "C2R" + row});
		}

		final Composite btnGroup = new Composite(group, SWT.NONE);
		...
		btn.addSelectionListener(widgetSelectedAdapter(e -> {
			final int currSize = table1.getItemCount();
			final int colCount = table1.getColumnCount();
			final CTableItem item = new CTableItem(table1, SWT.NONE);
			final String[] cells = new String[colCount];
		...
		btn.addSelectionListener(widgetSelectedAdapter(e -> {
			final CTableItem[] selectedItems = table1.getSelection();
			for (final CTableItem selectedItem : selectedItems) {
		...
		btn.addSelectionListener(widgetSelectedAdapter(e -> {
			final int currSize = table1.getColumnCount();
			final CTableColumn item = new CTableColumn(table1, SWT.NONE);
		...
		btn.addSelectionListener(widgetSelectedAdapter(e -> {
			final int colCount = table1.getColumnCount();
			if (colCount > 0) {
				final CTableColumn column = table1.getColumn(colCount - 1);
				column.dispose();
			}
		}));
		...
		final CTable table2 = new CTable(group, SWT.MULTI | SWT.FULL_SELECTION | SWT.BORDER);
		table2.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
		table2.setLinesVisible(true);
		for (final String element : itemText) {
			final CTableItem item = new CTableItem(table2, SWT.NONE);
