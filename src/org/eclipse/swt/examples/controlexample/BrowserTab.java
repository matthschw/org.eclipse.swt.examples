...
class BrowserTab extends Tab {
	 ...
	 */
	BrowserTab(final ControlExample instance) {
		super(instance);
	}
	...
	void createExampleGroup () {
		...
		this.browserGroup = new Group (this.exampleGroup, SWT.NONE);
		this.browserGroup.setLayout (new GridLayout ());
		this.browserGroup.setLayoutData (new GridData (SWT.FILL, SWT.FILL, true, true));
	...
	void createExampleWidgets () {
      ...
      style |= SWT.WEBKIT;
		...
		try {
			this.browser = new Browser (this.browserGroup, style);
		} catch (final SWTError e) { // Probably missing browser
			try {
				this.browser = new Browser (this.browserGroup, style & ~(SWT.WEBKIT));
			} catch (final SWTError e2) { // Unsupported platform
				this.errorMessage = e.getMessage();
				return;
			}
			final MessageBox dialog = new MessageBox(this.shell, SWT.ICON_WARNING | SWT.OK);
			final String resourceString = "WebKitNotFound";
		...
		this.lastText = this.lastUrl = null;
	}
	...
	void createStyleGroup () {
		...
		this.webKitButton = new Button (this.styleGroup, SWT.CHECK);
	...
	void disposeExampleWidgets () {
		...
		if (this.browser != null) {
			...
			if ((url.length() > 0) && !url.equals("about:blank")) { //$NON-NLS-1$
				...
				final String text = this.browser.getText();
				if (text.length() > 0) {
					this.lastText = text;
				}
			}
	...
	public static String getContents(final InputStream in) throws IOException {
		final StringBuilder sb= new StringBuilder(300);
	...
	void hookCustomListener (final String eventName) {
      ...
      return;
		...
		if (eventName == "AuthenticationListener") {
			this.browser.addAuthenticationListener(event -> this.log (eventName, event));
		}
		if (eventName == "CloseWindowListener") {
			this.browser.addCloseWindowListener (event -> this.log (eventName, event));
		}
		if (eventName == "LocationListener") {
			this.browser.addLocationListener (new LocationListener () {
				@Override
				public void changed(final LocationEvent event) {
					BrowserTab.this.log (eventName + ".changed", event);
				}
				@Override
				public void changing(final LocationEvent event) {
					BrowserTab.this.log (eventName + ".changing", event);
				}
			});
		}
		if (eventName == "OpenWindowListener") {
			this.browser.addOpenWindowListener (event -> this.log (eventName, event));
		}
		if (eventName == "ProgressListener") {
			this.browser.addProgressListener (new ProgressListener () {
				...
				public void completed(final ProgressEvent event) {
					BrowserTab.this.log (eventName + ".completed", event);
				}
			});
		}
		if (eventName == "StatusTextListener") {
			this.browser.addStatusTextListener (event -> this.log (eventName, event));
		}
		if (eventName == "TitleListener") {
			this.browser.addTitleListener (event -> this.log (eventName, event));
		}
		if (eventName == "VisibilityWindowListener") {
			this.browser.addVisibilityWindowListener (new VisibilityWindowListener () {
				...
				public void hide(final WindowEvent event) {
					BrowserTab.this.log (eventName + ".hide", event);
				}
				@Override
				public void show(final WindowEvent event) {
					BrowserTab.this.log (eventName + ".show", event);
				}
			});
		}
	}
	...
	void setExampleWidgetState () {
		...
		this.webKitButton.setSelection (this.browser == null ? false : (this.browser.getStyle () & SWT.WEBKIT) != 0);
		this.borderButton.setSelection (this.browser == null ? false : (this.browser.getStyle () & SWT.BORDER) != 0);
	}
}
